...
if (isset($_FILES["avatar"]["name"]) && $_FILES["avatar"]["tmp_name"] != ""){
	...
	if($fileSize > 12048576) {
		...
		header("location: ../message.php?msg=ERROR: Your image file was not jpg, gif or png or pdf type");
		exit();
...
if (isset($_FILES["photo"]["name"]) && isset($_POST["gallery"])){
	...
	$row = mysqli_fetch_row($query);
	if($row[0] > 99){
		header("location: ../message.php?msg=The demo system allows only 99 pictures total");
        exit();	
	}
	$gallery = preg_replace('#[^a-z 0-9,]#i', '', $_POST["gallery"]);
	...
	if($fileSize > 12048576) {
		header("location: ../message.php?msg=ERROR: Your image file was larger than 12mb");
		exit();	
	} else if (!preg_match("/\.(gif|jpg|png|pdf|jpeg)$/i", $fileName) ) {
	...
	$sql = "INSERT INTO photos(user, gallery, filename, uploaddate, format) VALUES ('$log_username','$gallery','$db_file_name',now(),'$fileExt')";
	$query = mysqli_query($db_conx, $sql);
	mysqli_close($db_conx);
