...
if (isset($_POST['action']) && $_POST['action'] == "status_post"){
	...
	$data_des = htmlentities($_POST['data_des']);
	$data_des = mysqli_real_escape_string($db_conx, $data_des);
	}
	else{
	$data_des='';
	}
	...
	$sql_user_1="Select id from users where username='$log_username' LIMIT 1";
	$sql_user_2="Select id from users where username='$account_name' LIMIT 1";
	$query_user_1 = mysqli_query($db_conx, $sql_user_1);
	//$result_user_1 = mysqli_fetch_row($query_user_1);
	$query_user_2 = mysqli_query($db_conx, $sql_user_2);
	//$result_user_2 = mysqli_fetch_row($query_user_2);
	$row_user_1 = mysqli_fetch_array($query_user_1,MYSQLI_ASSOC);
	$row_user_2 = mysqli_fetch_array($query_user_2,MYSQLI_ASSOC);
	$user_1 = $row_user_1["id"];
	$user_2 = $row_user_2["id"];
			
	$sql = "INSERT INTO status(account_name, author, type, data, postdate,user_wrote,user_wall,about,description) 
			VALUES('$account_name','$log_username','$type','$data',now(),'$user_1','$user_2','$typ','$data_des')";
	$query = mysqli_query($db_conx, $sql);
    ...
    $query = mysqli_query($db_conx, $sql); 
	$row = mysqli_fetch_row($query);
	if ($row[0] > 999) { // If they have 10 or more posts of type a
